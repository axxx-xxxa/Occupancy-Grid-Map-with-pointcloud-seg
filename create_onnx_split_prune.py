...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for k, v in w_dict['state_dict'].items():
            ...
            if "weight_orig" in k:
                # purne_weights[layer_name + ".weight"] = w_dict['state_dict'][layer_name + ".weight_orig"] * w_dict['state_dict'][layer_name + ".weight_mask"]
                purne_weights[k] = w_dict['state_dict'][layer_name + ".weight_orig"] * w_dict['state_dict'][layer_name + ".weight_mask"]

        for k, v in purne_weights.items():
            layer_name = ".".join(k.split(".")[:-1])
            if k in w_dict['state_dict']:
                w_dict['state_dict'][layer_name + '.weight'] = v

        print("")
        # w_dict_prune = {{'state_dict'}}
        # for k, v in w_dict['state_dict'].items():
        #     prune_layer = False
        ...
        #     if "weight_orig" in k:
        #         prune_layer = True
        #         prune_layer_name = layer_name
        #         print("prune layer !!!")
        #         w_dict_prune['state_dict'][layer_name + ".conv.weight"] = w_dict['state_dict'][layer_name + ".conv.weight_orig"] * w_dict['state_dict'][layer_name + ".conv.weight_mask"]
        #     if prune_layer_name not in k:
        #         prune_layer = False
        #         w_dict_prune['state_dict'][k] = v

        model.load_state_dict(w_dict['state_dict'], strict=False)
    ...
    onnx_path = os.path.join(FLAGS.root, "model_prune0.5.onnx")
