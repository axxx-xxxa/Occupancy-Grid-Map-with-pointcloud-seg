...
#                                                    [1, 2, 0])
    ...
    def do_range_projection(self):
        ...
        depth = np.linalg.norm(self.points, 2, axis=1)
        # print(f"---point {self.points[:1]}")
        # print(f"---depth {depth[:20]}")
        # get scan components
        scan_x = self.points[:, 0]
        scan_y = self.points[:, 1]
        scan_z = self.points[:, 2] 
        # scan_z = self.points[:, 2] + 0.7
        
        # print(f"{(np.array(self.points)>=0).all()}  {(np.array(scan_x)>=0).all()} {(np.array(scan_y)>=0).all()} {(np.array(scan_z)>=0).all()} ")
        ...
        # proj_y = [0 if x < 0 else x for x in proj_y]
        # proj_x = [0 if x < 0 else x for x in proj_x]
        # print("-"*50)
        # print(f"{len(points)} points ")
        # print(f"{len(order)} order ")
        # print(f"{(np.array(proj_x)>=0).all()} {(np.array(proj_y)>=0).all()} {(np.array(points)>=0).all()} {(np.array(order)>=0).all()}")
        # print(order[:20])
        # print(depth[:20])
        # print(indices[:20])
        # print(points[:20])
        ...
        # print(proj_y[:20])
        # print(proj_x[:20])
        # print(f"{len(proj_y)} proj_y {proj_y}, {len(proj_x)} proj_x {proj_x}, ")
        # print(f"proj_range[y,x] {self.proj_range[proj_y, proj_x]}")
        # print("-"*50)
